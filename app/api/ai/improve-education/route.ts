...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Improve this education description to be more professional: ${description}. Language: ${language}`;
    ...
    if (!geminiResponse.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to improve education from AI service' },
        { status: 500 }
      );
    }
